...
add_ori = torch.empty((2048, 2048)).cuda()
for i in range(2048):
    add_ori[i] = torch.ones(2048)*(i+1)

import numpy as np
def get_din_emb(c, input, normalize = True):
    num_pos_feats = c
    temperature = 10000
    # normalize = True
    scale = 2 * math.pi  # 圆周率
    ...
    if normalize:
        ...
        # print(y_embed)
        # print(x_embed)
    dim_t1 = torch.arange(num_pos_feats, dtype=torch.float32).cuda()
    # print(dim_t1)
...
class Space_block(nn.Module):
    def __init__(
        ...
        if self.emb_type == 'linear2':
            self.bn_emb = nn.BatchNorm2d(1)
        # self.lamdba = nn.ParameterList([nn.Parameter(torch.zeros(1)) for i in range(self.channel)])
        if self.cal_type=='normal':
            self.lamdba = nn.Parameter(torch.zeros(self.channel))
        elif self.cal_type=='normal_plus':
            self.conv =ConvModule(
                self.channel,
                self.channel,
                kernel_size=1,
                ...
                act_cfg=None)
            self.lamdba = nn.Parameter(torch.zeros(self.channel))
        elif self.cal_type=='normal_plus_2':
            self.conv =ConvModule(
                self.channel,
                self.channel,
                kernel_size=1,
                ...
                act_cfg=None)
            self.lamdba = nn.Parameter(torch.zeros(self.channel))
        elif self.cal_type=='normal_plus_3':
    ...
    def forward(self, x):
        ...
        nb, nc, w, h = shape
        add = None
        if self.emb_type == 'linear' or self.emb_type == 'noise':
            # add = add_ori[:w, :h]
            add = torch.empty(w,h).cuda()
            add = nn.init.kaiming_normal_(add, a=0, mode='fan_out', nonlinearity='relu')
            add = add.repeat(nb, nc, 1, 1)
        elif self.emb_type == 'linear2':
            add = self.bn_emb(add_ori[:w, :h].view(1, 1, w, h)).view(w,h)
            add = add.repeat(nb, nc, 1, 1)
        elif self.emb_type == 'sin':
            add = add_ori[:w, :h]
            add = get_din_emb(nc, add)
            add = add.repeat(nb,1,1,1)
        elif self.emb_type == 'sin_not_normalize':
            add = add_ori[:w, :h]
            add = get_din_emb(nc, add, normalize=False)
            add = add.repeat(nb, 1, 1, 1)


        if self.cal_type == 'normal':
            # product = torch.stack([ torch.eye(w).cuda()*self.lamdba[c] for c in range(self.channel) ])
            # product = product.repeat(nb,1,1,1)

            # for c in range(nc):
            #     # print(type(self.lamdba[c]), type(x))
            #     add[:, c, :, :] = add[:, c, :, :].clone()*self.lamdba[c]

            add = torch.einsum('c, ncij -> ncij', self.lamdba, add)
            out = x + add
            out = self.bn(out)
            if hasattr(self, 'act'):
                out = self.act(out)
            ...
            # product = torch.stack([torch.eye(w).cuda() * self.lamdba[c] for c in range(self.channel)])
            # product = product.repeat(nb, 1, 1, 1)
            # add = torch.einsum('ncik, nckj -> ncij', product, add)
            ...
            theta = torch.einsum('ncij -> nci', theta)
            theta = theta/h

            out = torch.einsum('nci, ncij -> ncij', theta, x) \
                  + torch.einsum('nci, ncij -> ncij', 1-theta, add)
            # out = theta * x + (1 - theta) * add
            out = self.bn(out)
            if hasattr(self, 'act'):
                out = self.act(out)
            ...
            # print('r')
            self.lamdba = self.conv1(x)
            self.lamdba = torch.einsum('ncij -> nci', self.lamdba)
            self.lamdba = self.lamdba / h

            add = torch.einsum('nci, ncij -> ncij', self.lamdba, add)
...
# shape = x.shape
# nb, nc, w, h = shape
# # add = torch.zeros(nb, nc, w, h).cuda()
# # add = torch.zeros(w, h).cuda()
# # for i in range(w):
# #     add[i] = torch.ones(h) * (i+1)
# add = add_ori[:w, :h]
# add = nn.init.kaiming_normal_(add, a=0, mode = 'fan_out', nonlinearity='relu')
# add = add.repeat(nb, nc, 1, 1)
#
...
# #     for c in range(nc):
# #         for i in range(w):
# #             add[b, c, i] = torch.ones(h) * (i+1)
# # print('@', nc, add.shape, self.channel)
# # for c in range(nc):
# #     # print(type(self.lamdba[c]), type(x))
# #     add[:, c, :, :] = add[:, c, :, :].clone()*self.lamdba[c]
#
...
class PIDNet_space2(BaseModule):
    """PIDNet backbone.
    ...
    def __init__(self,
        ...
        self.stem = self._make_stem_layer(in_channels, channels,
                                          num_stem_blocks, space_=space_block,
                                          space_emb=space_emb, space_cal=space_cal)
        # # activation = 'lrelu'
        # print(activation)
        # exit(0)
        ...
        # I Branch Context Not need to embed the Position Code
            ...
            self.i_branch_layers.append(
                nn.Sequential(
                        ...
                        self._make_layer(
                            ...
                            stride=2,
                            # _space=space_block,
                            # _space_cal=space_cal,
                            # _space_emb=space_emb,
                        ),
                )
            )
        ...
        if num_stem_blocks == 2:
            self.d_branch_layers = nn.ModuleList([
                ...
                self._make_layer(Bottleneck, channels, channels, 1, _space=space_block, _space_emb=space_emb, _space_cal=space_cal)
            ])
            channel_expand = 1
            spp_module = PAPPM
            dfm_module = LightBag
    ...
    def _make_stem_layer(self, in_channels: int, channels: int,
                         num_blocks: int, space_ = False, space_cal = 'normal', space_emb='linear') -> nn.Sequential:
        """Make stem layer.
        ...
        if space_:
            layers.append(Space_block(channels, cal_type=space_cal, emb_type=space_emb))
        layers.append(
            ConvModule(
                channels,
                channels,
    ...
    def _make_layer(self,
        ...
        for i in range(1, num_blocks):
            ...
            if _space==True and i == num_blocks - 1:
                layers.append(
                    Space_block(
                        in_channels=channels * block.expansion if stride != 1 or in_channels != channels * block.expansion else channels,
                        act_cfg_out=None if i == num_blocks - 1 else self.act_cfg,
                        cal_type=_space_cal,
                        emb_type= _space_emb,
                    )
    ...
    def forward(self, x: Tensor) -> Union[Tensor, Tuple[Tensor]]:
        ...
        # print(x_p.shape, out.shape, x_d.shape)
